...
### 👤 Perfil do Bruxo (Cloud Sync)
Salve seu progresso na nuvem de forma segura e acesse de qualquer lugar.
- Sincronização em tempo real de Nível, Coroas, Localização e Regiões Desbloqueadas.
- Login seguro via Google (Firebase Auth).
- Integração total com todos os outros Mestres (as IAs leem seu perfil automaticamente para dar dicas precisas).
...
- **Ícones:** Lucide React
- **Inteligência Artificial:** Google Gemini API (`@google/genai`) - Modelo *Gemini 3.1 Pro Preview* com **Google Search Grounding** para OCR de alta precisão, leitura de imagens borradas e conhecimento profundo atualizado.
- **Backend & Banco de Dados:** Firebase Authentication (Google Login) e Firebase Firestore.
...
- Um projeto no Firebase (para o banco de dados e login)
...
4. Configure o Firebase:
   Adicione as configurações do seu projeto Firebase no arquivo `firebase-applet-config.json` na raiz do projeto.
...
6. Abra o navegador em `http://localhost:3000` (ou a porta indicada no terminal).
